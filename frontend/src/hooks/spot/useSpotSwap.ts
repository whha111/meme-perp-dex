...
export function useSpotSwap() {
  ...
  /**
   * 计算报价
   * H-11 fix: 使用 bigint 计算 minAmountOut，避免浮点精度丢失
   */
  useEffect(() => {
    ...
    if (amountOutData && priceImpactData !== undefined && swapFee !== undefined) {
      const amountOutBigInt = amountOutData as bigint;
      ...
      // H-11 fix: 使用 bigint 精确计算最小输出（避免浮点精度丢失）
      // slippage 是百分比 (如 0.5 = 0.5%)，转为基点 (50 bps)
      const slippageBps = Math.round(slippage * 100); // 0.5% → 50 bps
      const minAmountOutBigInt = amountOutBigInt * BigInt(10000 - slippageBps) / 10000n;
      const minAmountOut = formatEther(minAmountOutBigInt);
