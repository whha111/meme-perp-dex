...
/**
 * @title Settlement
 * @notice 链上结算合约 - 中心化撮合 + 链上结算 (USDT 计价版本)
 * @dev Session Key 功能已移至 SessionKeyManager 合约
 */
contract Settlement is Ownable, ReentrancyGuard, EIP712 {
    ...
    uint256 public constant STANDARD_DECIMALS = 6;
    ...
    function settleBatch(MatchedPair[] calldata pairs) external nonReentrant {
        if (!authorizedMatchers[msg.sender]) revert Unauthorized();
    ...
    function _settlePair(MatchedPair calldata pair) internal {
        ...
        pairedPositions[pairId] = PairedPosition({
            ...
            openTime: block.timestamp, accFundingLong: 0, accFundingShort: 0, status: PositionStatus.ACTIVE
        });
    ...
    function closePair(uint256 pairId) external nonReentrant {
    ...
    function closePairsBatch(uint256[] calldata pairIds, uint256[] calldata exitPrices) external nonReentrant {
        if (!authorizedMatchers[msg.sender]) revert Unauthorized();
    ...
    function executeADL(uint256[] calldata pairIds, uint256[] calldata exitPrices) external nonReentrant {
        if (!authorizedMatchers[msg.sender]) revert Unauthorized();
        require(pairIds.length == exitPrices.length, "Length mismatch");
        for (uint256 i = 0; i < pairIds.length; i++) {
            if (pairedPositions[pairIds[i]].status == PositionStatus.ACTIVE) _closePair(pairIds[i], exitPrices[i]);
    ...
    function liquidate(uint256 pairId) external nonReentrant {
        PairedPosition storage pos = pairedPositions[pairId];
        if (pos.status != PositionStatus.ACTIVE) revert PositionNotActive();
        ...
        uint256 reward;
        address liqTrader;
        if (liqLong) { liqTrader = pos.longTrader; reward = (pos.longCollateral * 5) / 100; }
        else { liqTrader = pos.shortTrader; reward = (pos.shortCollateral * 5) / 100; }

        _settleProfit(pos, longPnL, shortPnL);
        if (reward > 0 && balances[liqTrader].available >= reward) {
            balances[liqTrader].available -= reward;
            balances[msg.sender].available += reward;
        }
        _updatePositionSize(pos);
        pos.status = PositionStatus.LIQUIDATED;
        emit Liquidated(pairId, liqTrader, msg.sender, reward);
    }
    ...
    function _settleFunding(uint256 pairId) internal {
        PairedPosition storage pos = pairedPositions[pairId];
        uint256 elapsed = block.timestamp - pos.openTime;
        if (elapsed == 0) return;
        int256 rate = fundingRates[pos.token];
        if (rate == 0) return;
        int256 funding = (int256(pos.size) * rate * int256(elapsed)) / int256(FUNDING_INTERVAL) / int256(PRECISION);
        pos.accFundingLong += funding;
        pos.accFundingShort -= funding;
    ...
    function getRemainingAmount(Order calldata order) external view returns (uint256) {
        uint256 filled = filledAmounts[getOrderHash(order)];
        return filled >= order.size ? 0 : order.size - filled;
    }
    ...
    function _hasLegacyPosition(address user) internal view returns (bool) {
        if (legacyPositionManager == address(0)) return false;
        (bool ok, bytes memory data) = legacyPositionManager.staticcall(abi.encodeWithSignature("getPosition(address)", user));
        if (!ok || data.length < 32) return false;
        uint256 size; assembly { size := mload(add(data, 64)) }
        return size > 0;
    }
    ...
    function _validateContractSpec(MatchedPair calldata pair) internal view {
        ...
        uint256 longNewPos = userPositionSizes[pair.longOrder.trader][pair.longOrder.token] + pair.matchSize;
        (, uint256 longMaxLev) = contractRegistry.getMarginRequirement(pair.longOrder.token, longNewPos);
        if (pair.longOrder.leverage > longMaxLev) revert LeverageTooHigh();
        if (longNewPos > spec.maxPositionSize) revert PositionLimitExceeded();

        uint256 shortNewPos = userPositionSizes[pair.shortOrder.trader][pair.shortOrder.token] + pair.matchSize;
        (, uint256 shortMaxLev) = contractRegistry.getMarginRequirement(pair.shortOrder.token, shortNewPos);
        if (pair.shortOrder.leverage > shortMaxLev) revert LeverageTooHigh();
        if (shortNewPos > spec.maxPositionSize) revert PositionLimitExceeded();

        _validatePriceDeviation(pair.longOrder, spec.maxPriceDeviation);
        _validatePriceDeviation(pair.shortOrder, spec.maxPriceDeviation);
    }

    function _validatePriceDeviation(Order calldata order, uint256 maxDev) internal view {
        if (order.orderType != OrderType.LIMIT || order.price == 0) return;
        uint256 cur = tokenPrices[order.token];
        if (cur == 0) return;
        uint256 dev = order.price > cur ? ((order.price - cur) * 10000) / cur : ((cur - order.price) * 10000) / cur;
        if (dev > maxDev) revert PriceDeviationTooLarge();
    }
