...
contract PositionManager is Ownable, ReentrancyGuard, IPositionManager {
    ...
    // 手续费配置
    uint256 public openFeeRate = 10; // 0.1% (10/10000)
    uint256 public closeFeeRate = 10; // 0.1%
    ...
    function _closePosition(address user, uint256 percentage) internal {
        ...
        // 核心：盈亏结算
        // ============================================================
        _settlePnL(user, closeCollateral, pnl, fee);
    ...
    function _openPositionToken(
        ...
        _validateAndProcessMargin(user, isLong, size, leverage, mode);
    ...
    function _validateAndProcessMargin(
        ...
        if (mode == MarginMode.CROSS) {
            _processCrossMargin(user, totalRequired);
        } else {
            _processIsolatedMargin(user, collateral, fee);
        }
    }
     ...
     */
    function _processCrossMargin(address user, uint256 totalRequired) internal {
        // 计算保证金和手续费
    ...
    function _processIsolatedMargin(address user, uint256 collateral, uint256 fee) internal {
    ...
    function _closePositionToken(address user, address token, uint256 percentage) internal {
        ...
        if (marginMode == MarginMode.CROSS) {
            _settlePnLCross(user, closeCollateral, pnl, fee);
        } else {
            _settlePnL(user, closeCollateral, pnl, fee);
        }
     ...
     * @param fee 平仓手续费
     */
    function _settlePnLCross(address user, uint256 collateral, int256 pnl, uint256 fee) internal {
        // 先从锁定保证金收取平仓手续费（与逐仓模式相同）
        if (fee > 0 && feeReceiver != address(0)) {
            // 确保手续费不超过保证金
            uint256 actualFee = fee > collateral ? collateral : fee;
            vault.collectFeeFromLocked(user, feeReceiver, actualFee);
