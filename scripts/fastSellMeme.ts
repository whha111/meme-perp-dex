...
async function main() {
  ...
  // 获取所有代币符号（并行）
  console.log("获取代币符号...");
  const symbols: Map<Address, string> = new Map();
  const symbolPromises = allTokens.map(async (token) => {
    try {
      ...
      symbols.set(token, sym);
    } catch {
      symbols.set(token, "???");
    }
  });
  await Promise.all(symbolPromises);
  console.log("代币符号获取完成\n");

  // Step 1: 并行扫描所有钱包
  console.log("--- Step 1: 并行扫描持仓 ---\n");

  const holdings: Holding[] = [];
  const BATCH_SIZE = 20; // 每批处理20个钱包

  for (let batchStart = 0; batchStart < wallets.length; batchStart += BATCH_SIZE) {
    const batchEnd = Math.min(batchStart + BATCH_SIZE, wallets.length);
    const batch = wallets.slice(batchStart, batchEnd);

    const batchPromises = batch.map(async (wallet, idx) => {
      const walletIndex = batchStart + idx;
    ...
    batchResults.forEach(h => holdings.push(...h));

    console.log("已扫描 " + batchEnd + "/" + wallets.length + " 个钱包...");
    await sleep(100); // 小延迟避免限流
  }
  ...
  for (const h of holdings) {
    console.log("  [" + h.walletIndex + "] " + h.symbol + ": " + formatEther(h.amount));
  }

  // 找出需要 gas 的钱包
  const uniqueWallets = [...new Set(holdings.map(h => h.walletIndex))];
  console.log("\n涉及 " + uniqueWallets.length + " 个钱包\n");
